...
In this section, the first explorations of the complete and incomplette datasets are shown.
...
In this section, some visualizations of the missing data patterns are offered.
...
In this section, we proceed with a more in depth comparison of the complete and incomplete dataset. Only the numeric variables with missing values active, height, weight and bmi are considered in all analyses. Smoke is featured in the comparison of means and distributions; all variables are included in the correlations.
...
The variances of the incompelte dataset are relatively similar to the complete dataset. The variance is overestimated the most in active heart rate.
...
In this section, we consider the ad-hoc imputations of Mean Substitution and Regression Imputation alongside the complete and incomplete dataset. As in the previous section, *age*, *intensity*, *sex* and *rest* are only shown in the correlations; and *smoke* is only shown in cases where a non-numeric variable makes sense.
...
In this section, the scientifically interesting model will be introduced and examined for the complete and incomplete datasets alongside the datasets created using the ad-hoc imputation methods.

For the scientifically interesting model, we attempt to predict active heart rate using resting heart rate, bmi, sex and if the respondents are smokers or not.
...
In this section, we will expand on the missing data mechanism in the data.
...
When looking ahead at multiple imputation, we can examine the proportion of usable cases for multiple imputation.

```{r}
UsableCases <- as.matrix(p$mr/(p$mr+p$mm))
round(UsableCases[c(2, 5, 7:9), ], 2)

```

The proportion of usable cases shows us that the imputation should, obviously, include the variables that are always present in the data. For the variables with missing values, *bmi* has the most usable cases for *smoke* while *smoke* has the most usable values for the other variables. However, this being a binary variable, it remains to be seen how useful it will be. For the numeric variables, *bmi* has a higher proportion of usable cases while *weight* seems to be the worst indicator with a very low poportion of usable cases.

```{r}
Outbound <- as.matrix(p$rm/(p$rm+p$rr))
round(Outbound[, c(2, 5, 7:9)], 2)
```

The outbound statistics leads to the same conclusions.

```{r}
round(flux(dfinc)[1:3], 2)
```

This is reflected in the high total outflux statistic for *smoke*. Hoever, *smoke* does suffer from a very low influx. *bmi* is also a good contender for a variable with missing values to use in imputation, drawing from its outflux. 
